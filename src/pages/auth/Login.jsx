...
// Import API functions for authentication and backend health check
const Login = () => {
  ...
  useEffect(() => {
    ...
    healthCheck().then((online) => {
      ...
      // Print status in browser console
      console.log("Backend status:", online ? "Online" : "Offline");
    });
  }, []);
  // Function that runs when user submits login form
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.message || "Login failed. Please try again now.");
